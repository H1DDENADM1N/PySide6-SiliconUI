...
class MySiliconApp(SiliconApplication):
    def __init__(self, *args, **kwargs):
        ...
        self.setWindowIcon(QIcon("examples/Gallery for siui/img/logo_new.png"))

        self.layerMain().addPage(
            ExampleHomepage(self), icon=SiGlobal.siui.iconpack.get("ic_fluent_home_filled"), hint="主页", side="top"
        )
        self.layerMain().addPage(
            ExampleIcons(self), icon=SiGlobal.siui.iconpack.get("ic_fluent_diversity_filled"), hint="图标包", side="top"
        )
        self.layerMain().addPage(
            RefactoredWidgets(self),
        ...
        self.layerMain().addPage(
            About(self), icon=SiGlobal.siui.iconpack.get("ic_fluent_info_filled"), hint="关于", side="bottom"
        )
