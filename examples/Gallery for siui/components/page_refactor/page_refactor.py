...
@contextmanager
def createDenseContainer(
    parent: SiDenseContainer, direction: QBoxLayout.Direction, side: Qt.Edges = Qt.LeftEdge | Qt.TopEdge
) -> SiDenseContainer:
    ...
    def __init__(self, *args, **kwargs):
        ...
        with self.titled_widgets_group as group:
            ...
            with createPanelCard(group, "条形进度条") as card:
                ...
                with createDenseContainer(card.body(), QBoxLayout.LeftToRight) as container:
                    ...
                    button_random_value.clicked.connect(lambda: progress_bar.setValue(int(random.random() * 1001)))
                    ...
                    button_loading.clicked.connect(lambda: progress_bar.setState(progress_bar.State.Loading))

                    button_processing = SiPushButtonRefactor.withText("处理")
                    button_processing.clicked.connect(lambda: progress_bar.setState(progress_bar.State.Processing))

                    button_paused = SiPushButtonRefactor.withText("暂停")
                    button_paused.clicked.connect(lambda: progress_bar.setState(progress_bar.State.Paused))

                    button_error = SiPushButtonRefactor.withText("错误")
                    button_error.clicked.connect(lambda: progress_bar.setState(progress_bar.State.Error))
        ...
        with self.titled_widgets_group as group:
            ...
            with createPanelCard(group, "进度按钮") as card:
                with createDenseContainer(card.body(), QBoxLayout.LeftToRight) as container:
                    ...
                    demo_progress_button_text.clicked.connect(
                        lambda: demo_progress_button_text.setProgress(random.random() * 1.3)
                    )
                    demo_progress_button_text.clicked.connect(
                        lambda: demo_progress_button_text_icon.setProgress(random.random() * 1.3)
                    )
                    demo_progress_button_text.clicked.connect(
                        lambda: demo_progress_button_icon.setProgress(random.random() * 1.3)
                    )
                    demo_progress_button_text.adjustSize()

                    demo_progress_button_text_icon = SiProgressPushButton(self)
                    demo_progress_button_text_icon.setSvgIcon(
        ...
        with self.titled_widgets_group as group:
            ...
            with createPanelCard(group, "区间指示器") as card:
                ...
                def test_func_1():
                    indicator.setStartIndex(
                        (indicator.startIndex() + 1) if indicator.startIndex() < indicator.nodeAmount() - 1 else 0
                    )

                def test_func_2():
                    indicator.setEndIndex(
                        (indicator.endIndex() + 1) if indicator.endIndex() < indicator.nodeAmount() - 1 else 0
                    )
